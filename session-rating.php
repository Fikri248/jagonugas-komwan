<?php
// session-rating.php
require_once __DIR__ . '/config.php';
require_once __DIR__ . '/db.php';
require_once __DIR__ . '/NotificationHelper.php';

if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

if (!isset($_SESSION['user_id']) || ($_SESSION['role'] ?? '') !== 'student') {
    header('Location: ' . BASE_PATH . '/login.php');
    exit;
}

$student_id = $_SESSION['user_id'];

// Koneksi
try {
    $pdo = (new Database())->getConnection();
} catch (Exception $e) {
    die('Database connection failed.');
}

$notif = new NotificationHelper($pdo);

// Ambil session_id dari GET / POST
$session_id = (int)($_GET['session_id'] ?? $_POST['session_id'] ?? 0);
if (!$session_id) {
    NotificationHelper::setError('Sesi tidak valid.');
    header('Location: ' . BASE_PATH . '/student-sessions.php');
    exit;
}

// Ambil data sesi
$stmt = $pdo->prepare("
    SELECT s.*, u.name AS mentor_name
    FROM sessions s
    JOIN users u ON s.mentor_id = u.id
    WHERE s.id = ? AND s.student_id = ?
");
$stmt->execute([$session_id, $student_id]);
$session = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$session) {
    NotificationHelper::setError('Sesi tidak ditemukan.');
    header('Location: ' . BASE_PATH . '/student-sessions.php');
    exit;
}

if ($session['status'] !== 'completed') {
    NotificationHelper::setError('Sesi ini belum selesai, tidak bisa diberi rating.');
    header('Location: ' . BASE_PATH . '/student-sessions.php');
    exit;
}

if (!empty($session['rating'])) {
    NotificationHelper::setError('Sesi ini sudah memiliki rating.');
    header('Location: ' . BASE_PATH . '/student-sessions.php');
    exit;
}

$errors = [];
$success = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $rating = (int)($_POST['rating'] ?? 0);
    $review = trim($_POST['review'] ?? '');

    if ($rating < 1 || $rating > 5) {
        $errors[] = 'Pilih rating antara 1 sampai 5 bintang.';
    }

    if (strlen($review) < 5) {
        $errors[] = 'Review minimal 5 karakter.';
    }

    if (empty($errors)) {
        try {
            $pdo->beginTransaction();

            // Simpan rating & review di tabel sessions
            $stmt = $pdo->prepare("UPDATE sessions SET rating = ?, review = ? WHERE id = ?");
            $stmt->execute([$rating, $review, $session_id]);

            // Update aggregate rating mentor (total_rating, review_count)
            $stmt = $pdo->prepare("
                UPDATE users
                SET total_rating = total_rating + ?,
                    review_count = review_count + 1
                WHERE id = ?
            ");
            $stmt->execute([$rating, $session['mentor_id']]);

            // Notifikasi ke mentor
            $studentName = $_SESSION['name'] ?? 'Mahasiswa';
            $notif->bookingCompleted($session['mentor_id'], $studentName, $session_id);

            $pdo->commit();

            NotificationHelper::setSuccess('Terima kasih! Rating dan review berhasil dikirim.');
            header('Location: ' . BASE_PATH . '/student-sessions.php');
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
            $errors[] = 'Terjadi kesalahan saat menyimpan rating.';
        }
    }
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Beri Rating Sesi - JagoNugas</title>
    <link rel="stylesheet" href="<?php echo BASE_PATH; ?>/style.css">
    <link rel="stylesheet" href="<?php echo BASE_PATH; ?>/style-mentor.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <style>
        .rating-page-container {
            max-width: 640px;
            margin: 2.5rem auto;
            padding: 1.5rem;
        }
        .rating-card {
            background: #ffffff;
            border-radius: 18px;
            padding: 1.75rem 1.75rem 2rem;
            box-shadow: 0 18px 40px rgba(15,23,42,0.18);
            border: 1px solid #e2e8f0;
        }
        .rating-header {
            display: flex;
            align-items: center;
            gap: 1rem;
            margin-bottom: 1.25rem;
        }
        .rating-mentor-avatar {
            width: 54px;
            height: 54px;
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
            font-weight: 800;
            color: #fff;
            background: linear-gradient(135deg,#6366f1,#ec4899);
            box-shadow: 0 10px 24px rgba(99,102,241,0.45);
        }
        .rating-stars {
            display: flex;
            gap: 0.4rem;
            margin: 1rem 0 0.75rem;
        }
        .rating-stars input {
            display: none;
        }
        .rating-stars label {
            font-size: 2rem;
            color: #e5e7eb;
            cursor: pointer;
            transition: transform 0.1s ease, color 0.1s ease;
        }
        .rating-stars input:checked ~ label,
        .rating-stars label:hover,
        .rating-stars label:hover ~ label {
            color: #fbbf24;
        }
        .rating-stars input:checked + label {
            color: #fbbf24;
        }
        .rating-textarea {
            width: 100%;
            min-height: 120px;
            border-radius: 12px;
            border: 1px solid #e2e8f0;
            padding: 0.75rem 0.9rem;
            font-size: 0.9rem;
            resize: vertical;
            outline: none;
        }
        .rating-textarea:focus {
            border-color: #6366f1;
            box-shadow: 0 0 0 1px rgba(99,102,241,0.15);
        }
        .rating-actions {
            display: flex;
            justify-content: flex-end;
            gap: 0.75rem;
            margin-top: 1.25rem;
        }
        .btn-secondary {
            background: #e5e7eb;
            color: #374151;
            border-radius: 999px;
            padding: 0.6rem 1.4rem;
            border: none;
            font-size: 0.9rem;
            font-weight: 600;
            cursor: pointer;
        }
        .btn-primary-solid {
            background: #6366f1;
            color: #fff;
            border-radius: 999px;
            padding: 0.6rem 1.6rem;
            border: none;
            font-size: 0.9rem;
            font-weight: 700;
            cursor: pointer;
            box-shadow: 0 10px 22px rgba(99,102,241,0.35);
        }
        .btn-primary-solid:hover {
            background: #4f46e5;
        }
        .rating-error {
            background: #fef2f2;
            border: 1px solid #fecaca;
            color: #b91c1c;
            border-radius: 12px;
            padding: 0.75rem 1rem;
            font-size: 0.85rem;
            margin-bottom: 0.75rem;
        }
        .rating-error ul {
            margin: 0.2rem 0 0;
            padding-left: 1.2rem;
        }
    </style>
</head>
<body>
<?php include 'student-navbar.php'; ?>

<div class="rating-page-container">
    <div class="rating-card">
        <div class="rating-header">
            <div class="rating-mentor-avatar">
                <?php echo strtoupper(substr($session['mentor_name'], 0, 1)); ?>
            </div>
            <div>
                <div style="font-size:0.8rem;color:#9ca3af;margin-bottom:0.15rem;">Beri Rating untuk Mentor</div>
                <h2 style="margin:0;font-size:1.25rem;color:#111827;">
                    <?php echo htmlspecialchars($session['mentor_name']); ?>
                </h2>
                <div style="font-size:0.85rem;color:#6b7280;margin-top:0.15rem;">
                    Sesi tanggal <?php echo date('d M Y, H:i', strtotime($session['created_at'])); ?>
                </div>
            </div>
        </div>

        <?php if (!empty($errors)): ?>
            <div class="rating-error">
                <strong>Terjadi kesalahan:</strong>
                <ul>
                    <?php foreach ($errors as $err): ?>
                        <li><?php echo htmlspecialchars($err); ?></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        <?php endif; ?>

        <form method="POST">
            <input type="hidden" name="session_id" value="<?php echo $session_id; ?>">

            <div>
                <div style="font-weight:600;font-size:0.95rem;color:#111827;">Pilih Rating</div>
                <div style="font-size:0.8rem;color:#9ca3af;margin-bottom:0.25rem;">1 = sangat buruk, 5 = sangat puas</div>

                <div class="rating-stars">
                    <?php for ($i = 5; $i >= 1; $i--): ?>
                        <input type="radio" id="star<?php echo $i; ?>" name="rating" value="<?php echo $i; ?>">
                        <label for="star<?php echo $i; ?>"><i class="bi bi-star-fill"></i></label>
                    <?php endfor; ?>
                </div>
            </div>

            <div style="margin-top:1rem;">
                <label for="review" style="font-weight:600;font-size:0.95rem;color:#111827;display:block;margin-bottom:0.35rem;">
                    Tulis Review
                </label>
                <textarea id="review" name="review" class="rating-textarea" placeholder="Ceritakan pengalamanmu dengan sesi ini..."></textarea>
            </div>

            <div class="rating-actions">
                <a href="<?php echo BASE_PATH; ?>/student-sessions.php" class="btn-secondary">
                    Batal
                </a>
                <button type="submit" class="btn-primary-solid">
                    Kirim Rating
                </button>
            </div>
        </form>
    </div>
</div>

</body>
</html>
